    ...
    if isinstance(src_data, dict):
        for key in src_data:
            if key not in dst_data:
                ...
                this_key = key
                # 判断是否同为dict类型，若是则再次调用校验方法
                if (src_data[this_key], dict) and isinstance(dst_data[this_key], dict):
                    ...
                    logging.info("要不试着修改下预取内容为dict字典？")
                    pass
    else:
